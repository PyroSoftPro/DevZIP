...
void print_usage() {
  std::cout
      << "DevZip CLI\n"
      << "Usage:\n"
      << "  devzip [--json] [--backend NAME] compress <source> [archive]\n"
...
int main(int argc, char** argv) {
  ...
  std::string backend_name = "best-of-two";
  ...
  try {
    ...
    if (command == "compress") {
      auto backend = make_backend_from_name(backend_name);
      ...
      const auto result = devzip::create_archive(source, archive, *backend);
      if (json_output) {
